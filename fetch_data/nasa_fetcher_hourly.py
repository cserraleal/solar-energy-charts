...
def fetch_hourly_data(dept_name, lat, lon, parameter):
    url = (
    ...
    try:
        ...
        for timestamp, value in records.items():
            ...
            if month_key not in grouped:
                grouped[month_key] = []
            if value != -999.0:
                grouped[month_key].append(round(value, 2))
...
def main():
    for dept in DEPARTMENTS:
        ...
        for param in PARAMETERS:
            result = fetch_hourly_data(nombre, lat, lon, param)
            if result:
                filename = f"{param.lower()}_hourly.json"
